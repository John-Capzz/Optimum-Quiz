...
export default function GameScreen({ question, timer, phase, reveal, myId, myAnswer, answeredCount, totalPlayers, questionIndex, totalQuestions, leaderboard, onAnswer }) {
  ...
  return (
    <div style={{ minHeight: "100vh", padding: "20px 24px", maxWidth: 1100, margin: "0 auto", animation: "fadeIn 0.3s ease" }}>
      ...
      <div style={{ display: "flex", alignItems: "center", gap: 16, marginBottom: 6 }}>
        ...
        <span style={{ fontSize: 12, color: "var(--muted)", fontFamily: "'Space Mono',monospace" }}>{answeredCount}/{totalPlayers} in</span>
      </div>
      ...
      <div style={{ display: "grid", gridTemplateColumns: "1fr 240px", gap: 24, alignItems: "start" }}>
        ...
        <div style={{ display: "flex", flexDirection: "column", gap: 16 }}>
          ...
          <div style={{ display: "grid", gridTemplateColumns: "1fr 1fr", gap: 12 }}>
            {question.options.map((opt, i) => {
              ...
              return (
                <button
                  key={i}
                  disabled={isReveal || myAnswer !== null}
                  onClick={() => !isReveal && myAnswer === null && onAnswer(i)}
          ...
          {isReveal && reveal?.results?.[myId] && (
            <div style={{ display: "flex", alignItems: "center", gap: 16, padding: "14px 20px", background: reveal.results[myId].correct ? "rgba(200,244,90,0.07)" : "rgba(255,77,106,0.07)", border: `1px solid ${reveal.results[myId].correct ? "rgba(200,244,90,0.2)" : "rgba(255,77,106,0.2)"}`, borderRadius: 12, animation: "scaleIn 0.3s ease" }}>
              ...
              <div>
                <p style={{ fontWeight: 700, color: reveal.results[myId].correct ? "var(--accent)" : "var(--red)" }}>{reveal.results[myId].correct ? "Correct!" : "Wrong"}</p>
                <p style={{ color: "var(--muted)", fontSize: 12 }}>{reveal.results[myId].correct ? `+${reveal.results[myId].pts} pts` : "No points"}</p>
              </div>
            </div>
          )}
        </div>
        ...
        <div style={{ background: "var(--surface)", border: "1px solid var(--border)", borderRadius: 16, padding: 20, position: "sticky", top: 20 }}>
          <p style={{ fontFamily: "'Space Mono',monospace", fontSize: 11, letterSpacing: 2, color: "var(--muted)", marginBottom: 12 }}>LEADERBOARD</p>
