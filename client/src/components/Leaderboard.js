...
function Confetti() {
  const pieces = Array.from({ length: 45 }, (_, i) => ({ id: i, x: Math.random() * 100, delay: Math.random() * 3, dur: 3 + Math.random() * 3, color: CONFETTI_COLORS[i % CONFETTI_COLORS.length], size: 6 + Math.random() * 8 }));
  return (
    <div style={{ position: "fixed", inset: 0, pointerEvents: "none", zIndex: 0, overflow: "hidden" }}>
      {pieces.map(p => <div key={p.id} style={{ position: "absolute", left: `${p.x}%`, top: -20, width: p.size, height: p.size, background: p.color, borderRadius: 2, animation: `confettiFall ${p.dur}s ${p.delay}s ease-in forwards` }} />)}
    </div>
  );
}

export default function Leaderboard({ leaderboard, myId, isHost, onReset, t }) {
  const [show, setShow] = useState(false);
  useEffect(() => { const tm = setTimeout(() => setShow(true), 300); return () => clearTimeout(tm); }, []);
    ...
    <div style={{ minHeight: "100vh", padding: "40px 24px", maxWidth: 680, margin: "0 auto", animation: "fadeIn 0.5s ease" }}>
      ...
      <div style={{ position: "relative", zIndex: 1 }}>
        <div style={{ textAlign: "center", marginBottom: 40 }}>
          ...
          <h1 style={{ fontSize: 44, fontWeight: 800, letterSpacing: "-2px", marginTop: 8 }}>{t.gameOver}</h1>
          {winner && <p style={{ fontSize: 18, color: "var(--muted)", marginTop: 10 }}><span style={{ color: "var(--gold)" }}>{winner.name}</span> {t.winsWith} <span style={{ color: "var(--accent)" }}>{winner.score.toLocaleString()} {t.pts}</span></p>}
          {myRank > 0 && <p style={{ marginTop: 8, color: "var(--muted)", fontSize: 14, fontFamily: "'Space Mono',monospace" }}>{t.youFinished} #{myRank}{myRank === 1 ? " 🎉" : myRank === 2 ? " 🥈" : myRank === 3 ? " 🥉" : ""}</p>}
        </div>
        ...
        <div style={{ marginBottom: 32 }}>
          <p style={{ fontFamily: "'Space Mono',monospace", fontSize: 11, letterSpacing: 2, color: "var(--muted)", marginBottom: 10 }}>{t.fullResults}</p>
          {leaderboard.map((p, i) => (
            <div key={p.id} style={{ display: "flex", alignItems: "center", gap: 14, padding: "13px 18px", borderRadius: 12, border: `1px solid ${p.id === myId ? "var(--accent)" : "var(--border)"}`, background: p.id === myId ? "var(--accent-dim)" : "var(--surface)", marginBottom: 8, animation: show ? `slideIn 0.4s ease ${i * 0.06}s both` : "none" }}>
              ...
              <span style={{ flex: 1, fontWeight: 600 }}>{p.name}{p.id === myId && <span style={{ fontSize: 11, fontFamily: "'Space Mono',monospace", color: "var(--accent)", marginLeft: 6 }}>{t.you}</span>}</span>
              ...
              <span style={{ fontSize: 11, color: "var(--muted)", fontFamily: "'Space Mono',monospace" }}>{t.pts}</span>
            </div>
          ))}
        </div>
        ...
        <div style={{ textAlign: "center", display: "flex", flexDirection: "column", alignItems: "center", gap: 14 }}>
          {isHost ? (
            <button style={{ background: "var(--accent)", color: "#000", border: "none", borderRadius: 12, padding: "16px 48px", fontSize: 15, fontWeight: 700, fontFamily: "'Space Mono',monospace", letterSpacing: 2, cursor: "pointer", animation: "glow 2s ease infinite" }} onClick={onReset}>{t.playAgain}</button>
          ) : (
            <p style={{ color: "var(--muted)", fontSize: 13, fontFamily: "'Space Mono',monospace" }}>{t.waitingForReset}</p>
          )}
